...
export class HeaderComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.commonService.localeEvent.subscribe((locale) =>
      this.translate.use(locale)
    );
    this.getAllNotification();
    this.apiService.currentUserData.subscribe((userData: any) => {
      this.token = userData.token
        ? userData.token
        : this.commonService.getUserData("token");
      this.userId = userData.user_id
        ? userData.user_id
        : this.commonService.getUserData("user_id");
      this.userName = userData.full_name
        ? userData.full_name
        : this.commonService.getUserData("full_name");
      this.userImage = userData.profile_image_url
        ? userData.profile_image_url
        : this.commonService.getUserData("profile_image_url");
      this.userType = userData.group_name
        ? userData.group_name
        : this.commonService.getUserData("group_name");
      this.clinic_id = userData.clinic_id
        ? userData.clinic_id
        : this.commonService.getUserData("clinic_id");
      this.settings = userData.settings
        ? userData.settings
        : this.commonService.getUserData("settings");
      this.clinics = userData.clinics
        ? userData.clinics
        : this.commonService.getUserData("clinics");
      this.localUserData =
        Object.keys(userData).length && userData.constructor === Object
          ? userData
          : this.commonService.getUserData();
      this.online_status = this.settings.hasOwnProperty("online_status")
        ? this.settings.online_status
        : "Offline";
  ...
  getAllNotification() {
    ...
    if (token != "") {
      this.notificationService.get({ limit: 5 }).subscribe((response: any) => {
        if (response.status) {
        ...
        this.userClinicsService.online(params).subscribe((response: any) => {
          if (response.status) {
            ...
            firebase
              ...
              .update({ status: this.online_status });
