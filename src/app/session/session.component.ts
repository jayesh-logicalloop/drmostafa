...
export class SessionComponent implements OnInit {
  ...
  getTotalItems() {
    this.sessionService.total({search: this.search, status: this.status}).subscribe(
      (response: any) => {
  ...
  getItems() {
    ...
    this.sessionService.get('', params).subscribe(
      (response: any) => {
  ...
  onSubmitAddForm() {
    if (this.addForm.valid) {
      ...
      this.sessionService.create(postData).subscribe(
        (response: any) => {
          this.modalService.close_modal("#addFormModal");
          this.alertService.show_alert(response.message);
          this.router.navigate(['/session/update/'+response.data]);
